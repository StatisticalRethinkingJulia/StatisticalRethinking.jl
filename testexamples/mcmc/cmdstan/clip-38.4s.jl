...
remove_names = filter(x->!in(x, [:height]), names(data))
...
stanmodel = Stanmodel(name="linear_regression",
  monitors = ["beta.1", "beta.2", "beta.3", "beta.4", "sigma"],
  model=lrmodel);
...
cnames = ["intercept", "beta[1]", "beta[2]", "beta[3]", "sigma"]
...
ols = lm(@formula(height ~ weight + age + male ), train_cut)
