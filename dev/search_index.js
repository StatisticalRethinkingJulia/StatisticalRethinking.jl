...

{
    ...
    "title": "m5.6s",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/05/m5.6s.jl\"Load Julia packages (libraries) needed  for the snippets in chapter 0using StatisticalRethinking\nusing CmdStan, StanMCMCChainCmdStan uses a tmp directory to store the output of cmdstanProjDir = rel_path(\"..\", \"scripts\", \"05\")\ncd(ProjDir)Read the milk datawd = CSV.read(rel_path(\"..\", \"data\", \"milk.csv\"), delim=\';\')\ndf = convert(DataFrame, wd);\ndcc = filter(row -> !(row[:neocortex_perc] == \"NA\"), df)\ndcc[:kcal_per_g] = convert(Vector{Float64}, dcc[:kcal_per_g])\ndcc[:log_mass] = log.(dcc[:mass])Show first 5 rowsfirst(dcc[[3, 7, 9]], 5)Define the Stan language modelm5_6_model = \"\ndata{\n    real kcal_per_g[17];\n    real log_mass[17];\n}\nparameters{\n    real a;\n    real bm;\n    real sigma;\n}\nmodel{\n    vector[17] mu;\n    sigma ~ uniform( 0 , 1 );\n    bm ~ normal( 0 , 1 );\n    a ~ normal( 0 , 100 );\n    for ( i in 1:17 ) {\n        mu[i] = a + bm * log_mass[i];\n    }\n    kcal_per_g ~ normal( mu , sigma );\n}\n\";Define the Stanmodel and set the output format to :mcmcchain.stanmodel = Stanmodel(name=\"m5_5_model\",\nmonitors = [\"a\", \"bm\", \"sigma\"],\n model=m5_6_model, output_format=:mcmcchain);Input data for cmdstanm5_6_data = Dict(\"N\" => size(dcc, 1),\n  \"kcal_per_g\" => dcc[:kcal_per_g],\n  \"log_mass\" => dcc[:log_mass]);Sample using cmdstanrc, chn, cnames = stan(stanmodel, m5_6_data, ProjDir, diagnostics=false,\n  CmdStanDir=CMDSTAN_HOME);Rethinking resultsrethinking_results = \"\n        mean    sd   5.5%  94.5% n_eff  Rhat\n        mean   sd  5.5% 94.5% n_eff Rhat\n a      0.70 0.06  0.61  0.79   926 1.00\n bm    -0.03 0.02 -0.07  0.01   986 1.00\n sigma  0.18 0.04  0.13  0.25   628 1.01\n\";Describe the drawsdescribe(chn)End of 05/5.5s.jlThis page was generated using Literate.jl."
},

{
    ...
    "title": "m8.1d",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/08/m8.1d.jl\"Load Julia packages (libraries) needed  for the snippets in chapter 0using StatisticalRethinking\nusing DynamicHMC, TransformVariables, LogDensityProblems, MCMCDiagnostics\nusing Parameters, ForwardDiffCmdStan uses a tmp directory to store the output of cmdstanProjDir = rel_path(\"..\", \"scripts\", \"08\")\ncd(ProjDir)"
},
...

{
    ...
    "title": "m8.1s",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/08/m8.1s.jl\"Load Julia packages (libraries) needed  for the snippets in chapter 0using StatisticalRethinking\nusing CmdStan, StanMCMCChain\ngr(size=(500,500));CmdStan uses a tmp directory to store the output of cmdstanProjDir = rel_path(\"..\", \"scripts\", \"08\")\ncd(ProjDir)"
},

{
    "location": "08/m8.1s/#snippet-5.1-1",
    "page": "m8.1s",
    "title": "snippet 5.1",
    "category": "section",
    "text": "d = CSV.read(rel_path(\"..\", \"data\", \"rugged.csv\"), delim=\';\');\ndf = convert(DataFrame, d);\n\ndcc = filter(row -> !(ismissing(row[:rgdppc_2000])), df)\ndcc[:log_gdp] = log.(dcc[:rgdppc_2000])\ndcc[:cont_africa] = Array{Float64}(convert(Array{Int}, dcc[:cont_africa]))\ndcc[:rugged] = convert(Array{Float64}, dcc[:rugged])\nfirst(dcc[[:rugged, :cont_africa, :log_gdp]], 5)Define the Stan language modelm_8_1_model = \"\ndata{\n    int N;\n    vector[N] log_gdp;\n    vector[N] cont_africa;\n    vector[N] rugged;\n    vector[N] rugged_cont_africa;\n}\nparameters{\n    real a;\n    real bR;\n    real bA;\n    real bAR;\n    real sigma;\n}\nmodel{\n    vector[N] mu = a + bR * rugged + bA * cont_africa + bAR * rugged_cont_africa;\n    sigma ~ uniform( 0 , 10 );\n    bAR ~ normal( 0 , 10 );\n    bA ~ normal( 0 , 10 );\n    bR ~ normal( 0 , 10 );\n    a ~ normal( 0 , 100 );\n    log_gdp ~ normal( mu , sigma );\n}\n\";Define the Stanmodel and set the output format to :mcmcchain.stanmodel = Stanmodel(name=\"m_8_1_model\",\nmonitors = [\"a\", \"bR\", \"bA\", \"bAR\", \"sigma\"],\nmodel=m_8_1_model, output_format=:mcmcchain);Input data for cmdstanm_8_1_data = Dict(\"N\" => size(dcc, 1),\n\"log_gdp\" => dcc[:log_gdp],  \"rugged\" => dcc[:rugged],\n\"cont_africa\" => dcc[:cont_africa],\n\"rugged_cont_africa\" => dcc[:rugged] .* dcc[:cont_africa] );Sample using cmdstanrc, chn, cnames = stan(stanmodel, m_8_1_data, ProjDir, diagnostics=false,\n  summary=true, CmdStanDir=CMDSTAN_HOME);Result rethinkingrethinking = \"\n       mean   sd  5.5% 94.5% n_eff Rhat\na      9.22 0.14  9.00  9.46   282    1\nbR    -0.21 0.08 -0.33 -0.08   275    1\nbA    -1.94 0.24 -2.33 -1.59   268    1\nbAR    0.40 0.14  0.18  0.62   271    1\nsigma  0.96 0.05  0.87  1.04   339    1\n\"Describe the drawsdescribe(chn)End of 08/m8.1s.jlThis page was generated using Literate.jl."
},
...

{
    ...
    "title": "m8.5s",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/08/m8.5s.jl\"Load Julia packages (libraries) needed  for the snippets in chapter 0using StatisticalRethinking\nusing CmdStan, StanMCMCChain\ngr(size=(500,500));CmdStan uses a tmp directory to store the output of cmdstanProjDir = rel_path(\"..\", \"scripts\", \"08\")\ncd(ProjDir)Define the Stan language modelm_8_5_model = \"\ndata{\n  int N;\n  vector[N] y;\n}\nparameters{\n  real sigma;\n  real alpha;\n  real a1;\n  real a2;\n}\nmodel{\n  real mu;\n  a1 ~ normal(0, 10);\n  a2 ~ normal(0, 10);\n  mu = a1 + a2;\n  y ~ normal( mu , sigma );\n}\n\";Define the Stanmodel and set the output format to :mcmcchain.stanmodel = Stanmodel(name=\"m_8_5_model\", monitors = [\"a1\", \"a2\", \"mu\", \"sigma\"],\nmodel=m_8_5_model, output_format=:mcmcchain);Input data for cmdstanm_8_5_data = Dict(\"N\" => 100, \"y\" => rand(Normal(0, 1), 100));Sample using cmdstanrc, chn, cnames = stan(stanmodel, m_8_5_data, ProjDir, diagnostics=false,\n  summary=true, CmdStanDir=CMDSTAN_HOME);\n\nrethinking = \"\n       mean   sd   5.5% 94.5% n_eff Rhat\na1    -0.08 7.15 -11.34 11.25  1680    1\na2    -0.05 7.15 -11.37 11.19  1682    1\nsigma  0.90 0.07   0.81  1.02  2186    1\n\";Describe the drawsdescribe(chn)End of 08/m8.1s.jlThis page was generated using Literate.jl."
},

{
    ...
    "title": "m8.8s",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/08/m8.8s.jl\"Load Julia packages (libraries) needed  for the snippets in chapter 0using StatisticalRethinking\nusing CmdStan, StanMCMCChain\ngr(size=(500,500));CmdStan uses a tmp directory to store the output of cmdstanProjDir = rel_path(\"..\", \"scripts\", \"08\")\ncd(ProjDir)"
},

{
    "location": "08/m8.8s/#snippet-8.25-1",
    "page": "m8.8s",
    "title": "snippet 8.25",
    "category": "section",
    "text": "N = 100                                                    # individuals\nheight  = rand(Normal(10,2), N) ;              # sim total height of each\nleg_prop = rand(Uniform(0.4,0.5), N);      # leg as proportion of heightsim left leg as proportion + errorleg_left = leg_prop .* height .+  rand(Normal( 0 , 0.02 ), N);sim right leg as proportion + errorleg_right = leg_prop .* height .+  rand(Normal( 0 , 0.02 ), N);combine into data framedf =  DataFrame(height=height, leg_left = leg_left, leg_right = leg_right);Show first 5 rowsfirst(df, 5)Define the Stan language modelm_5_8_model = \"\ndata{\n    int N;\n    real height[N];\n    real leg_right[N];\n    real leg_left[N];\n}\nparameters{\n    real a;\n    real bl;\n    real br;\n    real sigma;\n}\nmodel{\n    vector[N] mu;\n    sigma ~ cauchy( 0 , 1 );\n    br ~ normal( 2 , 10 );\n    bl ~ normal( 2 , 10 );\n    a ~ normal( 10 , 100 );\n    for ( i in 1:100 ) {\n        mu[i] = a + bl * leg_left[i] + br * leg_right[i];\n    }\n    height ~ normal( mu , sigma );\n}\n\";Define the Stanmodel and set the output format to :mcmcchain.stanmodel = Stanmodel(name=\"m_5_8_model\", monitors = [\"a\", \"br\", \"bl\", \"sigma\"],\n  model=m_5_8_model, output_format=:mcmcchain);Input data for cmdstanm_8_8_data = Dict(\"N\" => size(df, 1), \"height\" => df[:height],\n    \"leg_left\" => df[:leg_left], \"leg_right\" => df[:leg_right]);Sample using cmdstanrc, chn, cnames = stan(stanmodel, m_8_8_data, ProjDir, diagnostics=false,\n  summary=false, CmdStanDir=CMDSTAN_HOME);Describe the drawsdescribe(chn)Plot the density of posterior drawsplot(chn)Autocorrelationautocorplot(chn)End of 08/m8.8s.jlThis page was generated using Literate.jl."
},
...

{
    ...
    "title": "m10.04d",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/10/m10.04d.jl\"Load Julia packages (libraries) needed  for the snippets in chapter 0using StatisticalRethinking\nusing DynamicHMC, TransformVariables, LogDensityProblems, MCMCDiagnostics\nusing Parameters, ForwardDiff, LinearAlgebraCmdStan uses a tmp directory to store the output of cmdstanProjDir = rel_path(\"..\", \"scripts\", \"10\")\ncd(ProjDir)"
},
...

{
    ...
    "title": "m12.6s",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/12/m12.6s.jl\"using StatisticalRethinking\nusing CmdStan, StanMCMCChain\n\nProjDir = rel_path(\"..\", \"scripts\", \"12\")\ncd(ProjDir)\n\nd = CSV.read(rel_path( \"..\", \"data\",  \"Kline.csv\"), delim=\';\');\nsize(d) # Should be 10x5New col log_pop, set log() for population datad[:log_pop] = map((x) -> log(x), d[:population]);\nd[:society] = 1:10;\nfirst(d[[:culture, :population, :log_pop, :society]], 5)\n\nm12_6 = \"\ndata {\n    int N;\n    int N_societies;\n    int total_tools[N];\n    real logpop[N];\n    int society[N];\n}\nparameters{\n    real a;\n    real bp;\n    vector[N_societies] a_society;\n    real<lower=0> sigma_society;\n}\nmodel{\n    vector[N_societies] mu;\n    sigma_society ~ cauchy( 0 , 1 );\n    a_society ~ normal( 0 , sigma_society );\n    bp ~ normal( 0 , 1 );\n    a ~ normal( 0 , 10 );\n    for ( i in 1:N ) {\n        mu[i] = a + a_society[society[i]] + bp * logpop[i];\n        mu[i] = exp(mu[i]);\n    }\n    total_tools ~ poisson( mu );\n}\n\";Define the Stanmodel and set the output format to :mcmcchain.stanmodel = Stanmodel(name=\"m12.6\",  model=m12_6, output_format=:mcmcchain);Input data for cmdstanm12_6_data = Dict(\"N\" => size(d, 1),\"N_societies\" => 10,\n\"total_tools\" => d[:total_tools], \"logpop\" => d[:log_pop],\n\"society\" => d[:society]);Sample using cmdstanrc, chn, cnames = stan(stanmodel, m12_6_data, ProjDir, diagnostics=false, summary=false, CmdStanDir=CMDSTAN_HOME);Describe the drawsdescribe(chn)This page was generated using Literate.jl."
},

{
    ...
    "title": "m12.6s2",
    "category": "page",
    "text": "EditURL = \"https://github.com/StanJulia/StatisticalRethinking.jl/blob/master/scripts/12/m12.6s2.jl\"using StatisticalRethinking\nusing CmdStan, StanMCMCChain\n\nProjDir = rel_path(\"..\", \"scripts\", \"12\")\n\nd = CSV.read(rel_path( \"..\", \"data\",  \"Kline.csv\"), delim=\';\');\nsize(d) # Should be 10x5New col log_pop, set log() for population datad[:log_pop] = map((x) -> log(x), d[:population]);\nd[:society] = 1:10;\n\nfirst(d, 5)\n\nm12_6_2 = \"\n  data {\n    int N;\n    int T[N];\n    int N_societies;\n    int society[N];\n    int P[N];\n  }\n  parameters {\n    real alpha;\n    vector[N_societies] a_society;\n    real bp;\n    real<lower=0> sigma_society;\n  }\n  model {\n    vector[N] mu;\n    target += normal_lpdf(alpha | 0, 10);\n    target += normal_lpdf(bp | 0, 1);\n    target += cauchy_lpdf(sigma_society | 0, 1);\n    target += normal_lpdf(a_society | 0, sigma_society);\n    for(i in 1:N) mu[i] = alpha + a_society[society[i]] + bp * log(P[i]);\n    target += poisson_log_lpmf(T | mu);\n  }\n  generated quantities {\n    vector[N] log_lik;\n    {\n    vector[N] mu;\n    for(i in 1:N) {\n      mu[i] = alpha + a_society[society[i]] + bp * log(P[i]);\n      log_lik[i] = poisson_log_lpmf(T[i] | mu[i]);\n    }\n    }\n  }\n\";Define the Stanmodel and set the output format to :mcmcchain.stanmodel = Stanmodel(name=\"m12.6.2\",  model=m12_6_2,\noutput_format=:mcmcchain);Input data for cmdstanm12_6_2_data = Dict(\"N\" => size(d, 1), \"T\" => d[:total_tools],\n\"N_societies\" => 10, \"society\" => d[:society], \"P\" => d[:population]);Sample using cmdstanrc, chn, cnames = stan(stanmodel, m12_6_2_data, ProjDir,\ndiagnostics=false, summary=false, CmdStanDir=CMDSTAN_HOME);Describe the drawsdescribe(chn)This page was generated using Literate.jl."
},
